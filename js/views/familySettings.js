/* ============================================================
   ...
   (which holds the vision/identity). Sections:
     1. Home Location
     2. Family Members & Support People   (moved here from North Star)
     3. Travel / Worldschool Mode
     4. Faith Integration                  (moved here from Settings)
     5. Child Mobility & Independence       (per child)

   Everything auto-saves (debounced write-behind) and there is an
   explicit Save. Data lives on the family / child records and syncs
   per-family via the existing RLS-scoped Supabase columns.
   ============================================================ */
...
export function renderFamilySettings(container) {
  ...
  // Working copies (committed via auto-save / Save).
  const loc = { ...(fam.location || {}) };
  // Working copies — ensure every person has a stable id + access fields so the
  // permission engine can reference them (Owner / Contributor + permissions).
  ...
  // Rhythm: stored config layered over sensible defaults inferred from location.
  ...
  const SECTIONS = [
    { id: "location", icon: "📍", title: "Home Location", blurb: "Your general area, so North Star can suggest real-world learning near you.", body: locationBody },
    { id: "relationships", icon: "👪", title: "Family Members & Support People", blurb: "The real people in your child's world — North Star only ever refers to those you add.", body: relBody },
    { id: "travel", icon: "🌏", title: "Travel / Worldschool Mode", blurb: "Turn on when you're away, for local experiences or destination-based projects.", body: travelBody },
    { id: "faith", icon: "🕊️", title: "Faith Integration", blurb: "Optional — weave your family's faith values into projects and reflections.", body: faithBody },
    { id: "rhythm", icon: "🗓️", title: "Family Rhythm", blurb: "Your school year, days and hours — so North Star scales to your week, never overfills it.", body: rhythmBody },
  ];
  ...
  const gatherFamily = () => ({
    ...
    // Extended faith detail (synced to cloud once migration 0012 is applied).
    faith: {
    ...
    // Family Rhythm config (cloud-syncs once migration 0013 is applied).
    rhythm: { ...rhythm },
  });

  const statusEl = container.querySelector("#fs-autosave");
  let saveTimer;
  const commit = () => { setFamily(gatherFamily()); };
  const autosave = () => {
  ...
  /* ---------------- Home Location ---------------- */
  const locInput = container.querySelector("#home-loc");
  const locDetail = container.querySelector("#loc-detail");
  ...
  // Typing freely (without picking) keeps the display string but clears the structured bits.
  ...
  container.querySelector("#loc-clear").addEventListener("click", () => {
  ...
  /* ---------------- Family Members ---------------- */
  const relList = container.querySelector("#rel-list");
  const renderRel = () => {
    ...
    relList.querySelectorAll("[data-rel]").forEach(row => {
      ...
      row.querySelector('[data-rk="remove"]').addEventListener("click", () => { rel.splice(i, 1); renderRel(); autosave(); });
      ...
      row.querySelector(`[data-invite="${i}"]`)?.addEventListener("click", async () => {
        ...
        try {
          commit();   // persist the person's access settings before inviting
          const fam = getState().family || {};
          // Resolve "all children" (empty childIds) to explicit ids for the grant.
  ...
  container.querySelector("#rel-add").addEventListener("click", () => {
  ...
  /* ---------------- Travel ---------------- */
  const modeSel = container.querySelector("#travel-mode");
  const destWrap = container.querySelector("#travel-dest-wrap");
  const destList = container.querySelector("#dest-list");
  const renderDest = () => {
    ...
    destList.querySelectorAll("[data-dest]").forEach(row => {
      ...
      row.querySelector('[data-dk="remove"]').addEventListener("click", () => { travel.destinations.splice(i, 1); renderDest(); autosave(); });
    });
  };
  ...
  container.querySelector("#dest-add").addEventListener("click", () => { addDestination(); renderDest(); });
  ...
  /* ---------------- Faith ---------------- */
  const faithOn = container.querySelector("#faith-on");
  const faithFields = container.querySelector("#faith-fields");
  faithOn.addEventListener("change", () => {
    faith.enabled = faithOn.checked;
    faithFields.classList.toggle("hidden", !faith.enabled);
  });
  const bindFaith = (id, key) => container.querySelector(id)?.addEventListener("input", e => { faith[key] = e.target.value; });
  ...
  // Re-size the notes box when the Faith section / its fields become visible.
  const faithSection = container.querySelector('[data-acc="faith"]');
  const refitFaith = () => container.querySelector("#faith-notes")?.dispatchEvent(new Event("input"));
  faithSection?.addEventListener("acc:open", refitFaith);
  faithOn.addEventListener("change", refitFaith);

  /* ---------------- Family Rhythm ---------------- */
  const previewEl = container.querySelector("#ry-preview");
  const paintRhythm = () => {
  ...
  // Hemisphere "auto" → infer from location country via defaultRhythm.
  container.querySelector("#ry-hemi")?.addEventListener("change", (e) => {
    if (e.target.value === "auto") {
      delete rhythm.hemisphere;
      const d = defaultRhythm(loc.country || fam.location?.country);
      rhythm.hemisphere = d.hemisphere; // keep a concrete value for the engine
    } else {
...
function relRow(r, i, children = []) {
  ...
  const childOn = (id) => !childIds.length || childIds.includes(id);   // empty = all
  return `
