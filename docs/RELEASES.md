...
|------------|--------------|------------------|-------------------|----------|----------|----------|
| 2026-07-30 | 1a34c1e | **0038_milestone_estimated_minutes** | **ai** (deeper milestones + honest estimatedMinutes + start-today first mission; age-12 rebucketed to substantial) | frontend + migration + edge fn | ✅ on main (release-check green); migration applied (additive nullable col, advisors unaffected); ai fn deployed (401 unauth = verify_jwt live); col verified present; 62 unit tests pass. Fixes shallow/prep-only output (Noah "Adventure Keeper"). estimatedMinutes now round-trips (repo.js) + drives capacity estimator. | ⏳ live: needs a real regeneration to confirm missions are meatier + mission 1 is do-today; foundation for daily-load slice |
| 2026-07-24 | (unmerged: feat/child-lodge-slice1 @0686612) | **0037_calendar_feeds** | **calendar-ics** (fetch+parse external ICS server-side) | migration + edge fn | ⚠️ **--allow-unmerged OVERRIDE** — deployed from branch 18 ahead of main. Justified: strictly additive (new `calendar_feeds` table + new `calendar-ics` fn), nothing on main references either → zero stale-frontend risk. Migration applied (4 RLS policies via is_family_member; advisors show calendar_feeds clean, no new lints); fn deployed (401 unauth = verify_jwt live). **Merge PR #9 to main ASAP to reconcile.** | ⏳ live: needs an authenticated parent to add a real Google/iCloud ICS URL and see events overlay |
| 2026-07-19 | 1084643 | 0033 (prev) | **ai** (G6 hard-cap: finalTarget ≤ availableProjectMinutes; two modes) | edge fn | ✅ 51/51 unit incl. hard-invariant sweep; live A–F via real parent JWT, all respect the ceiling — A(12h/0/med)→std medium est540≤2016 ok; **B(12h/2/med)→ capped 672 not 750, est450 ok+note**; **C(4h/2/med)&D(4h/2/large)→ insufficient_capacity, 0 gen ~3s, defer message (old 300-min-from-112 defect gone)**; E(24h/0/large)→ large est1080≤8064 ok; F(0.5h/0/med)→ insufficient (42 avail). Sub-small "compact" tier tried→rejected on evidence (generator floor ~150–210min ≈ small floor). Test data cleaned (0/0/0/0). **Capacity-ceiling problem closed** | — |
